...
Tests:
- R015-T01: Run with no discovered test files and verify explicit non-zero failure output.
- R015-T02: Run with one or more discovered test files and verify invocation proceeds.

R020  Statement: Execute all discovered Bats files in one lane.
Design: Invoke `bats` with all discovered test files.
Tests:
- R020-T01: Verify `bats` receives all discovered files.
