...
R010  Statement: Wrapper loads matchy runbook profile before delegation.
Design: Export `RUNBOOK_REPO_ROOT` and source `runner/config/runbook/matchy.env` prior to `exec`.
Tests:
...
Tests:
- R015-T01: Verify wrapper source delegates to `load_requirements_generic.sh` with `"$@"`.
