...
R010  Statement: Run a fixed checklist of seven numbered check scripts.
Design: Launch, in order: `00_verify_requirements_traceability.sh`, `04_run_dependency_freshness_checks.sh`, `05_run_unit_tests.sh`, `06_run_security_checks.sh`, `07_run_av_checks.sh`, `10_run_mutation_tests.sh`, and `11_run_fuzz.sh`.
...
Tests:
- R020-T01: Stub one child to exit non-zero and others to exit zero; verify all seven results are reported.

R025  Statement: Print per-script pass/fail summary lines as each check completes.
Design: Emit exactly seven `✅ PASS:` or `❌ FAIL:` lines, one per checklist script basename, in completion order as soon as each background job finishes (before the overall summary line).
Tests:
- R025-T01: When all children succeed, verify seven PASS lines.
...
Design: Exit `0` with an overall `✅ PASS:` line when all seven checks succeed; exit non-zero with an overall `❌ FAIL:` line when any check fails.
Tests:
...
Design: The seven child check scripts must not invoke or reference `12_run_all_checks_parallel.sh`; each child remains an independent numbered entrypoint.
Tests:
- R040-T01: Grep the seven child scripts for `run_all_checks_parallel` and verify no matches.

R045  Statement: Report continuous aggregate progress while checks are running.
Design: Render a textual progress bar with completed/total counts and percentage that starts at `0/7 (0%)`, updates periodically while child checks are still running, and reaches `7/7 (100%)` before the final overall summary line.
Design: Keep per-check completion lines intact; progress rendering must not suppress any `✅ PASS:` or `❌ FAIL:` check result line.
Tests:
- R045-T01: Use staggered child runtimes and verify progress output includes intermediate states before all seven checks complete.
- R045-T02: Verify final output includes `7/7 (100%)` before the overall PASS/FAIL summary line.

R046  Statement: Export outer lane count for nested parallel runners.
Design: Before launching child checks, export `PARALLEL_LANES="${#CHECKS[@]}"` so nested parallel runners divide inner job counts and keep total concurrency near `hw.ncpu`.
Tests:
- R046-T01: Stub `05_run_unit_tests.sh` to print `PARALLEL_LANES` and verify the exported value equals seven.
...
Design: On SIGINT/SIGTERM, recursively terminate the full descendant tree for each launched lane (not just direct children), then fall back to `pgrep -f` matches for `${SCRIPT_DIR}/<check-script>` to catch re-parented workers such as mutmut, hypothesis, and clamscan.
...
R060  Statement: Report per-check elapsed time and identify the long pole.
Design: Write each lane start epoch to `${PARALLEL_CHECKS_REPORT_DIR}/<script-stem>.start` at launch.
Design: Append `(<Ns>)` to each per-check `✅ PASS:` or `❌ FAIL:` line when the lane completes.
Design: Before the overall summary line, print `Timing: wall <wall>s; long pole <script> (<Ns>)` where long pole is the completed lane with the greatest elapsed seconds.
Tests:
- R060-T01: Use staggered child runtimes (`sleep 1` default, one `sleep 3`) and verify per-check `(Ns)` suffixes, the timing line names the slow stub as long pole, and wall time is below sequential execution time.
- R060-T02: When one child fails, verify its FAIL line includes duration and the timing summary still appears before the overall FAIL line.

## Changelog

- 2026-05-20: Added R060 long-pole and per-check timing reporting.
- 2026-05-20: Reswizzled from valve `13_run_all_checks_parallel.sh` for Matchy check scripts and `PARALLEL_LANES` composition.
