...
Applies to `tests/t05_run_shell_unit_tests.sh`. Thin pointer that selects the matchy runbook profile and delegates the lane to the runner golden via the shared shim.
...
Design: Call `delegate_golden "tests/t05_run_shell_unit_tests.sh" "$@"` so the shim execs the runner golden with arguments passed through unchanged.
Tests:
