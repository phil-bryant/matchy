# t06 run python unit tests Wrapper Requirements

## Scope

Applies to `tests/t06_run_python_unit_tests.sh`. Thin pointer that selects the matchy runbook profile and delegates the lane to the runner golden via the shared shim.
...
Design: Call `delegate_golden "tests/t06_run_python_unit_tests.sh" "$@"` so the shim execs the runner golden with arguments passed through unchanged.
Tests:
