# t08 run fuzz tests Wrapper Requirements

## Scope

Applies to `tests/t08_run_fuzz_tests.sh`. Thin pointer that selects the matchy runbook profile and delegates the lane to the runner golden via the shared shim.
...
Design: Call `delegate_golden "tests/t08_run_fuzz_tests.sh" "$@"` so the shim execs the runner golden with arguments passed through unchanged.
Tests:
