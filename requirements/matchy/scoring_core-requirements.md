...
for invalid/unquantizable inputs.
Tests:
- R760-T01: Half-up rounding behavior is verified at half-cent boundaries (`tests/py/test_scoring_core.py`).
- R760-T02: Whole-dollar to cent scaling is verified (`tests/py/test_scoring_core.py`).
- R760-T03: Unquantizable decimal input returns `None` (`tests/py/test_scoring_core.py`).

R761  Statement: `_extract_money_cents` parses money-like tokens and normalizes them into cent values.
Design: Parse optional dollar-prefixed/grouped numeric tokens, convert parsed values through `_decimal_to_cents`, and
drop unparseable fragments.
Tests:
- R761-T01: Extracted cent sets include grouped/decimal/signed tokens and exclude invalid fragments (`tests/py/test_scoring_core.py`).
...
- 2026-06-06: Added R760-R761 helper requirements for decimal-cent quantization and money-token extraction.
