# t10 verify filevault encryption test Wrapper Requirements

## Scope

Applies to `tests/t10_verify_filevault_encryption_test.sh`. Thin pointer that selects the matchy runbook profile and delegates the lane to the runner golden via the shared shim.
...
Design: Call `delegate_golden "tests/t10_verify_filevault_encryption_test.sh" "$@"` so the shim execs the runner golden with arguments passed through unchanged.
Tests:
