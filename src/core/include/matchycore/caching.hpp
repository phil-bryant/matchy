...
{ //R520: Normalize ranked candidates into cache rows (score, reasons, cached metadata).
 nlohmann::json RankedCandidateCacheRows(const std::vector<RankedCandidate> &ranked_candidates);

 //R525: Order-independent SHA-256 fingerprint of full candidate cache rows.
 std::string CandidateSetHash(const nlohmann::json &candidate_cache_rows);

 //R530: Deterministic fallback hash from sorted candidate message ids.
 std::string CandidateMessageIdHash(const std::vector<std::string> &message_ids);

 //R020: Cached response (skipped=true) when the prior verdict still applies, else nullopt.
