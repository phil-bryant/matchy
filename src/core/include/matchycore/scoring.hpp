...
{ //R001 R010: Normalize text to lowercase with non [a-z0-9 whitespace] bytes replaced by spaces.
 ...
 //R015: Token overlap over long tokens (length > 2): intersection divided by the larger token-set size.
 double TokenOverlap(const std::string &left, const std::string &right);

 //R760: Parse a decimal string and return absolute integer cents using half-up rounding, or nullopt on invalid input.
 std::optional<long long> DecimalToCents(const std::string &value);

 //R761: Extract money-like numeric tokens (optionally $-prefixed, comma-grouped, 0-2 decimals) as integer cents.
 std::set<long long> ExtractMoneyCents(const std::string &text);

 //R020: Exact integer-cents hint between the transaction amount and any money token in candidate text.
 double AmountHintScore(const std::string &amount, const EmailCandidate &candidate);

 //R025: Binary long-token overlap between transaction text and sender text.
 double SenderHintScore(const std::string &transaction_text, const std::string &sender);

 //R030: Binary hint when a long (>=6 chars, non-digit) transaction token appears in compacted candidate text.
 double CompactMerchantHintScore(const std::string &transaction_text, const std::string &candidate_text);

 //R035: Map absolute hour distance to the documented proximity buckets.
 double TimeProximityScore(TimePoint txn_time, TimePoint received_at);

 //R040: Tokenize normalized text into long tokens preserving repeats for term frequency.
 std::vector<std::string> RelevanceTokens(const std::string &value);

 //R041: Count how many corpus documents contain each long token at least once.
 std::map<std::string, int> DocumentFrequencies(const std::vector<std::string> &documents);

 //R042: Smoothed BM25 inverse document frequency.
 double InverseDocumentFrequency(int corpus_size, int document_frequency);

 //R043: Okapi BM25 relevance of a query against one document using corpus statistics.
 ...
 //R044: Saturate a non-negative BM25 score into the unit interval.
 double Bm25Relevance(double score, double saturation = 4.0);

 //R045: Subset-sum reachability over positive integer-cent amounts within an inclusive tolerance band.
 bool SubsetSumReachable(const std::vector<long long> &amounts_cents, long long target_cents,
                         long long tolerance_cents = 0);

 //R046: Reconciliation signal when smaller line items can sum to the transaction total.
 ...
 //R005 R047: Rank candidates by the weighted heuristic blend, sorted by descending score (stable).
