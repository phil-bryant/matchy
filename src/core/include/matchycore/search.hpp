...
{ //R803: Deterministic search terms from counterparty/description text (short, numeric, duplicate tokens filtered).
 std::vector<std::string> ExtractSearchTerms(const std::string &description, const std::string &counterparty_name,
                                             int max_terms = 2);

 //R805: Inclusive from/to date window suffix around the transaction date; empty when window_days <= 0.
 std::string DateWindowSuffix(TimePoint txn_date, int window_days);

 //R804: Scoped Mailcart queries per term/field combination, optionally suffixed with the date window.
 ...
 //R040: De-duplicate by message_id preserving order, dropping id-less rows, capped at limit.
 ...
 { public:
  ...
  //R040: Execute the fallback chain (body+date -> subject+date -> body -> empty) with early stop.
  std::vector<EmailCandidate> SearchCandidates(const TransactionInput &txn, const std::string &transaction_id);
  //R800: True while the transient-failure cooldown window is active.
