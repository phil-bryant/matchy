#!/usr/bin/env bats
# Thin pointer contract tests for tests/t09_run_dynamic_security_tests.sh.
...
setup() {
  ...
  POINTER="${REPO_ROOT}/tests/t09_run_dynamic_security_tests.sh"
}
...
@test "delegates to the mapped runner golden with argument passthrough" {
  ...
  run grep -F 'delegate_golden "tests/t09_run_dynamic_security_tests.sh" "$@"' "$POINTER"
  [ "$status" -eq 0 ]
}
