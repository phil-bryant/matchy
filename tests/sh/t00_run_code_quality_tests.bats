#!/usr/bin/env bats
# Thin pointer contract tests for tests/t00_run_code_quality_tests.sh.
# Fully offline: they inspect the pointer's wiring text only and never execute the delegated lane.

setup() {
  REPO_ROOT="$(cd "$(dirname "$BATS_TEST_FILENAME")/../.." && pwd -P)"
  POINTER="${REPO_ROOT}/tests/t00_run_code_quality_tests.sh"
}
...
@test "delegates to the mapped runner golden with argument passthrough" {
  ...
  run grep -F 'delegate_golden "tests/t00_run_code_quality_tests.sh" "$@"' "$POINTER"
  [ "$status" -eq 0 ]
}
