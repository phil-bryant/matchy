...
# the matchy-owned C++ core sanitizer lane. These assert its ASan/UBSan contract.
...
@test "lane rebuilds the C++ core with sanitizers enabled" {
  #R001-T01: sanitizer build configuration (MATCHYCORE_SANITIZE=ON, Debug) and build.
  run grep -q 'DMATCHYCORE_SANITIZE=ON' "$SCRIPT"
  [ "$status" -eq 0 ]
...
@test "lane reruns the unit suite under sanitizers with halt-on-error" {
  ...
  run grep -q 'UBSAN_OPTIONS=halt_on_error=1 "\${BUILD_DIR}/matchycore_tests"' "$SCRIPT"
  [ "$status" -eq 0 ]
