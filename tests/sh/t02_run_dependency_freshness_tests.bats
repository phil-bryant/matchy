#!/usr/bin/env bats
# Thin pointer contract tests for tests/t02_run_dependency_freshness_tests.sh.
...
setup() {
  ...
  POINTER="${REPO_ROOT}/tests/t02_run_dependency_freshness_tests.sh"
}
...
@test "delegates to the mapped runner golden with argument passthrough" {
  ...
  run grep -F 'delegate_golden "tests/t02_run_dependency_freshness_tests.sh" "$@"' "$POINTER"
  [ "$status" -eq 0 ]
}
