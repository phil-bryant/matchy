#!/usr/bin/env bats
# Thin pointer contract tests for tests/t10_verify_filevault_encryption_test.sh.
...
setup() {
  ...
  POINTER="${REPO_ROOT}/tests/t10_verify_filevault_encryption_test.sh"
}
...
@test "delegates to the mapped runner golden with argument passthrough" {
  ...
  run grep -F 'delegate_golden "tests/t10_verify_filevault_encryption_test.sh" "$@"' "$POINTER"
  [ "$status" -eq 0 ]
}
