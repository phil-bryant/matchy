...
# the matchy-owned C++ core unit lane. These assert its build + run contract.
...
@test "lane runs the Catch2 C++ core unit suite" {
  #R005-T01: runs matchycore_tests and reports a passing marker.
  run grep -q '"\${BUILD_DIR}/matchycore_tests"' "$SCRIPT"
  [ "$status" -eq 0 ]
