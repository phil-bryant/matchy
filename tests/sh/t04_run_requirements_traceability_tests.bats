#!/usr/bin/env bats
# Thin pointer contract tests for tests/t04_run_requirements_traceability_tests.sh.
...
setup() {
  ...
  POINTER="${REPO_ROOT}/tests/t04_run_requirements_traceability_tests.sh"
}
...
@test "delegates to the mapped runner golden with argument passthrough" {
  ...
  run grep -F 'delegate_golden "tests/t04_run_requirements_traceability_tests.sh" "$@"' "$POINTER"
  [ "$status" -eq 0 ]
}
