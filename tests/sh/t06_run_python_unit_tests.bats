#!/usr/bin/env bats
# Thin pointer contract tests for tests/t06_run_python_unit_tests.sh.
...
setup() {
  ...
  POINTER="${REPO_ROOT}/tests/t06_run_python_unit_tests.sh"
}
...
@test "delegates to the mapped runner golden with argument passthrough" {
  ...
  run grep -F 'delegate_golden "tests/t06_run_python_unit_tests.sh" "$@"' "$POINTER"
  [ "$status" -eq 0 ]
}
