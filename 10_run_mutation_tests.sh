...
if ! "$PYTHON_BIN" -m mutmut --version >/dev/null 2>&1; then
  echo "mutmut is required in matchy-venv but was not found."
...
if [ -d "${REPO_ROOT}/mutants" ]; then
  MUTANTS_TRASH="$(mktemp -d "${HOME}/.Trash/matchy_mutants_XXXXXX")"
...
set +e
export PATH="${REPO_ROOT}/matchy-venv/bin:${PATH}"
