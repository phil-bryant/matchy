...
verify_python_test_lane_coverage() {
    local requirements_file="$1" source_list_file="$2"
...
source_list_file = Path(sys.argv[1])
missing_file = Path(sys.argv[2])
repo_root = Path.cwd()
missing = []
if source_list_file.is_file():
    for raw in source_list_file.read_text(encoding="utf-8").splitlines():
        source = raw.strip()
        if not source:
            continue
...
with missing_file.open("w", encoding="utf-8") as handle:
    for source, expected in sorted(set(missing)):
        handle.write(f"{source}|{expected}\n")
PY
    if [ ! -s "$missing_python_tests_file" ]; then
        return 0
    fi
    echo "❌ FAIL (python-test-lane): missing Python tests for ${requirements_file}:"
    while IFS='|' read -r source_file expected_test_file; do
        [ -n "$source_file" ] || continue
        echo "  - ${source_file} -> expected ${expected_test_file}"
    done < "$missing_python_tests_file"
    return 1
}
...
verify_single_pair_with_tests() {
    ...
    if ! verify_python_test_lane_coverage "$requirements_file" "$source_list_file"; then
        status=1
    fi
