...
def _purge_pycache_under(mutants_root: Path) -> None:
    ...
    for cache_dir in mutants_root.rglob("__pycache__"):
        if not cache_dir.is_dir():
            continue
        trash = Path.home() / ".Trash" / f"matchy_pycache_{stamp}_{cache_dir.parent.name}"
        trash.parent.mkdir(parents=True, exist_ok=True)
        if trash.exists():
            trash = Path.home() / ".Trash" / f"matchy_pycache_{stamp}_{cache_dir.parent.name}_{os.getpid()}"
...
def _run_mutant_pytest(python: Path, root: Path, mutant_name: str, tests: list[str]) -> int:
    ...
    pytest_args = [
        str(python),  # keep venv shim path; do not resolve() or pytest may miss venv site-packages
        "-m",
        "pytest",
        "-x",
        "-q",
...
def main(argv: list[str] | None = None) -> int:
    ...
    root = _repo_root()
    python = Path(os.environ.get("MATCHY_PYTHON", root / "matchy-venv/bin/python"))
